...
def overlay_path(path, height=0.05):
    for i in range(len(path) - 1):
        start = np.array([path[i][0], path[i][1], height]) / 100
        end = np.array([path[i+1][0], path[i+1][1], height]) / 100
...
def three_d_viz():
    ...
    terrain_id = p.createMultiBody(0, terrain_shape, basePosition=[3, 3, 0], baseOrientation=[0, 0, -0.707, 0.707])
    print("Here 2")
