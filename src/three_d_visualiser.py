...
# yes, I put this path directly here, instead of storing it in a file.
...
def follow_waypoints(robot_id, waypoints, step_size=0.05, threshold=0.1, z_offset=0.1):
    """
    Function to make the robot follow waypoints while adapting to the terrain.
    ...
    Args:
        ...
        z_offset: Safe height above the terrain surface.
    """
    for waypoint in waypoints:
        while True:
            ...
            # Use rayTest to adjust z based on terrain height
            ray_start = [new_x, new_y, current_z + 10]
            ray_end = [new_x, new_y, current_z - 10]
            ray_result = p.rayTest(ray_start, ray_end)

            if ray_result and ray_result[0][0] != -1:
                # Extract terrain height from ray test result
                terrain_z = ray_result[0][3][2]
                # Adjust z to be slightly above terrain height
                new_z = terrain_z + z_offset
            else:
                print("RayTest failed. Maintaining current height.")
                new_z = current_z

            # Smooth transition for z to avoid abrupt jumps
            new_z = 0.9 * current_z + 0.1 * new_z

            # Set the robot's new position
            p.resetBasePositionAndOrientation(
                robot_id, [new_x, new_y, new_z], p.getQuaternionFromEuler([0, 0, 0])
            )

            # Debug information
            print(f"Moving to: x={new_x}, y={new_y}, z={new_z}")
